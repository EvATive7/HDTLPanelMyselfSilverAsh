...
local enablePMA = true
-- 判断 args.args['model'] .. "assets/disablePMA" 是否存在
local disablePMAFile = io.open(args.args['model'] .. "assets/disablePMA", "r")
if disablePMAFile ~= nil then
    enablePMA = false
    disablePMAFile:close()
end

local modelRuntimeParameters = { hittest = settings.hittest, pma = enablePMA }
...
ipc.addPanelItem(
    ...
    function(v)
        modelRuntimeParameters.hittest = v
        settings.hittest = v
        settings:save()
    end,
...
local idle_audios_eve = (function()
    ...
    for i, v in ipairs(def.idle) do
        vs[i] = audio.register(args.args['audio'] .. v)
        vs[i]:loop(false)
    end
...
local interact_audios = (function()
    ...
    local vs = {}
    for i, v in ipairs(def.interact) do
        vs[i] = audio.register(args.args['audio'] .. v)
        vs[i]:loop(false)
    end
...
(function()
    ...
    ipc.addPanelItem(
        ...
        function(v)
            v = math.max(v, settings.stopDistance)
            settings.startDistance = v
            walker.startDistance = v
            settings:save()
        end,
    ...
    ipc.addPanelItem(
        ...
        function(v)
            settings.stopDistance = v
            walker.stopDistance = v
            settings:save()
        end,
...
(function()
    ...
    enterStateThen["interact"] = (function()
        if hasSpecial then
            return function()
                model.once(math.random(5) == 1 and "Special" or "Interact")
                interact_audios()
                special = false
            end
        else
